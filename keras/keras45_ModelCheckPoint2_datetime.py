...
# date_now = datetime.datetime.now()          # 컴퓨터가 제공하는 시간과 동일
# print(date_now)
# date_time = date_now.strftime('_%m%d_%H%M%S_')
# print(date_time)
...
modelpath = filepath + str(mm) + str(sec) + filename #'_'.join([filepath, 'k45_', date_time, filename])
print(modelpath)
...
hist = model.fit(x_train, y_train, batch_size = 32, epochs = 10, validation_split = 0.2, callbacks = [es, cp])
