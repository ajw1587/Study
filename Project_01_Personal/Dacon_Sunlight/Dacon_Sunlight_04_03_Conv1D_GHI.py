...
for q in q_list:
  ...
  cp = ModelCheckpoint(filepath = file_path, save_best_only = True, monitor = 'val_loss')
  model.compile(loss = lambda y_test, y_predict: quantile_loss(q, y_test, y_predict), optimizer = opti, metrics = ['mae'])
  model.fit(x_train, y_train, epochs = 1000, batch_size = 40, validation_data = (x_val, y_val), callbacks = [es, reduce_lr, cp])
