...
for q in q_list:
    model = mymodel()
    file_path = "../data/modelcheckpoint/Sunlight/Sunlight_03/Sunlight_03_03_" + str(q) + "_{epoch:02d}.hdf5"
    ...
    model.fit(x_train, y_train1, epochs = 20, batch_size = 49, validation_data = (x_val, y_val1), callbacks = [es, reduce_lr, cp])
...
for q in q_list:
    model = mymodel()
    file_path = "../data/modelcheckpoint/Sunlight/Sunlight_03/Sunlight_03_04_" + str(q) + "_{epoch:02d}.hdf5"
    cp = ModelCheckpoint(filepath = file_path, monitor = 'loss', save_best_only = True, mode = 'auto')
    model.compile(loss=lambda y_test, y_predict: quantile_loss(q, y_test, y_predict), optimizer='adam',
                  metrics = [lambda y_test, y_predict: quantile_loss(q, y_test, y_predict)])
    model.fit(x_train, y_train2, epochs = 20, batch_size = 49, validation_data = (x_val, y_val2), callbacks = [es, reduce_lr, cp])
    y_pred_test = pd.DataFrame(model.predict(x_pred_test, batch_size = 49))
