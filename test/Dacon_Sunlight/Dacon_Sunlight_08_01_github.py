...
def compile(a, x_train, y_train, x_val, y_val):
    for q in quantile_list:
        ...
        file_path=f'../data/modelcheckpoint/Sunlight/base_time/dacon_model_time{i}-{a}-{q}.hdf5'
...
def predict(a, x_train, y_train, x_val, y_val, x_test):
    x=list()
    for q in quantile_list:
        model=load_model(f'../data/modelcheckpoint/Sunlight/base_time/dacon_model_time{i}-{a}-{q}.hdf5', compile=False)
...
test=list()
for i in range(81):
    a=pd.read_csv('../data/csv/Sunlight_generation/test/' + str(i) + '.csv')
    a=preprocess_data(a)
    test.append(a)
test=pd.concat(test)
test=np.array(test)
...
sub.to_csv('../Sunlight/0125/Sunlight_0126_.csv', index=False)