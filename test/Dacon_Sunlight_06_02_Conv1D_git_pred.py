...
def train_data(x_train, x_test, y_train, y_test, x_val, y_val, x_pred):
    ...
    for q in quantiles:
        print("(q_%.1f) my_model start : >>>>>>>>>>>>>>>>>>>>>>" %q) 
        #2. Modeling
        model = load_model("../data/modelcheckpoint/Sunlight/Sunlight_0124_" + str(q) + ".hdf5", compile = False)
        # model.summary()
        #3. Compile, Train
        # cp_save = "../data/modelcheckpoint/Sunlight/Sunlight_0124_" + str(q) + "_{epoch:02d}_{val_loss:.4f}.hdf5"
        # cp = ModelCheckpoint(filepath=cp_save, monitor='val_loss', save_best_only=True, verbose=1, mode='min')
