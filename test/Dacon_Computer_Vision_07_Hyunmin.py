...
from sklearn.model_selection import train_test_split, StratifiedKFold # KFold보다 데이터셋의 대표성을 더 잘 나타내준다.
...
for train_index, test_index in skf.split(train2, train['digit']) : # >>> x, y
    path = '../data/modelcheckpoint/Computer_Vision/Computer_Vision_best_model_hyunmin.hdf5'
    ...
    model.load_weights('../data/modelcheckpoint/Computer_Vision/Computer_Vision_best_model_hyunmin.hdf5')
    ...
    print(sub)
    sub.to_csv('../data/modelcheckpoint/Computer_Vision/submission_data_hyunmin.csv', index=False)
