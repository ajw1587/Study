...
def windowed_dataset(series, window_size, batch_size, shuffle_buffer):
    series = tf.expand_dims(series, axis=-1)                            # 차원 추가 (x, 1)
    ds = tf.data.Dataset.from_tensor_slices(series)                     # numpy나 list를 tensor dataset으로 변경
    ds = ds.window(window_size + 1, shift=1, drop_remainder=True)       # 시계열 데이터로 변환, + 1은 예측값
    ds = ds.flat_map(lambda w: w.batch(window_size + 1))                # w.batch(a): a만큼 잘라서 평평하게 만들어줌
    ds = ds.shuffle(shuffle_buffer)                                     # Shuffle
    ds = ds.map(lambda w: (w[:-1], w[1:]))                              # 훈련데이터, 결과데이터 분할
    return ds.batch(batch_size).prefetch(1)                             # batch_size만큼 분할
...
def solution_model():
    ...
    with open('sunspots.csv') as csvfile:
      ...
      next(reader)
      for row in reader:
        sunspots.append(row[2])   # YOUR CODE HERE
        time_step.append(row[0])  # YOUR CODE HERE

    series = np.array(sunspots, dtype = np.float)   # YOUR CODE HERE
    ...
    model.fit(train_set, epochs = 200, batch_size = 32, callbacks = [es, reduce_lr])
